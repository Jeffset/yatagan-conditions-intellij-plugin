...
abstract class YceConditionQualifierMixin(node: ASTNode) : ASTWrapperPsiElement(node) {
    override fun getReference(): PsiReference? {
        return ReferenceImpl(this)
    }

    private class ReferenceImpl(
        element: YceConditionQualifierMixin,
    ) : PsiReferenceBase<YceConditionQualifierMixin>(element) {

        override fun resolve(): PsiElement? {
            val file = element.containingFile as YceFile
            val context = file.context ?: return null

            val simpleName = element.text
